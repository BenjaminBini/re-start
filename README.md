...
- **task backends**: choose between local storage (no setup required), Todoist, or Google Tasks in the settings.
  - **Todoist**: to get your api token, open the Todoist website and click your profile in the top left. then go to "Settings" > "Integrations" > "Developer" > "Copy API Token". the 'x tasks' text becomes a clickable link to Todoist.
  - **Google Tasks**: click the "Sign in with Google" button in settings. uses the same authentication as Google Calendar.
  - **local storage**: no setup required, tasks are stored in your browser.
- **smart date parsing**: when adding tasks, type natural language dates like 'tomorrow', 'next friday', 'jan 15', or '12/31'. the app will highlight the parsed date in real-time as you type, making it easy to see what date will be set.
- **Google Calendar**: click the "Sign in with Google" button in settings to enable calendar events. uses the same authentication as Google Tasks, so signing in once enables both features.
- **background images**: toggle backgrounds on/off in settings. adjust opacity with the slider to control how visible the background is (higher = more visible). backgrounds auto-refresh daily with a new image, or click "Force refresh background" in settings to get a new image immediately.
- you can force a refresh of the weather, tasks, and calendar widgets by clicking the top left panel labels. the tasks refresh applies to whichever backend you have selected (local, Todoist, or Google Tasks).
